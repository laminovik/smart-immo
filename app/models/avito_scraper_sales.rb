...
class AvitoScraperSales
  ...
  def fetch_results
    ...
    url = "http://www.avito.ma/fr/f%C3%A8s/appartements-%C3%A0_vendre"
    #url de navigation dans les pages de résultats des ventes
    nav_url="http://www.avito.ma/fr/k%C3%A9nitra/appartements-%C3%A0_vendre?o="
     ...
     #data=Nokogiri::HTML(open(nav_url+k.to_s))
      ...
      items.each do |item|
        ...
        if  !price_el.nil?
          ...
          if info[4].text=="Type: Appartements, Offre"
            type=1
          end
          ...
          #ActiveRecord::Base.transaction do
            Sale.create(city_id: @city.id, surface: surface, rooms: rooms, price: price,
          ...
          puts "#{rooms} pièces de #{surface} m² à "+district.to_s+ " au prix de #{price} DH"
       else
